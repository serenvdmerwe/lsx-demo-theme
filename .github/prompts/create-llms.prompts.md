...
Create a comprehensive `llms.txt` file that serves as an entry point for LLMs to understand and navigate the repository effectively. The file must comply with the llms.txt specification and be optimised for LLM consumption while remaining human-readable.
...
Organise files into logical H2 sections such as:
...
8. Organise content logically for easy consumption
